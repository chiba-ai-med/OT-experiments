...
index_pos_test_data <- Reduce(intersect,
	list(
		which(pos_test_data$X > 500),
		which(pos_test_data$X < 600),
		which(pos_test_data$Y > 320),
		which(pos_test_data$Y < 340)))

index_neg_test_data <- Reduce(intersect,
	list(
		which(neg_test_data$X > 500),
		which(neg_test_data$X < 600),
		which(neg_test_data$Y > 320),
		which(neg_test_data$Y < 340)))

pos_test_data <- pos_test_data[index_pos_test_data, ]
neg_test_data <- neg_test_data[index_neg_test_data, ]
...
pos_train_data <- sapply(unique(pos_train_data$Id), function(x){
	index_pos_train_data <- Reduce(intersect,
		list(
			which(pos_train_data$Id == x),
			which(pos_train_data$X > 500),
			which(pos_train_data$X < 600),
			which(pos_train_data$Y > 320),
			which(pos_train_data$Y < 340)))
	merge(pos_train_data[index_pos_train_data, 2:4],
...
neg_train_data <- sapply(unique(neg_train_data$Id), function(x){
	index_neg_train_data <- Reduce(intersect,
		list(
			which(neg_train_data$Id == x),
			which(neg_train_data$X > 500),
			which(neg_train_data$X < 600),
			which(neg_train_data$Y > 320),
			which(neg_train_data$Y < 340)))
	merge(neg_train_data[index_neg_train_data, 2:4],
